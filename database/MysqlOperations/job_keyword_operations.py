...
def insert_job_model(**kwargs):
    query = f"""
            INSERT INTO job_model_keywords (keyword_name, job_id, created_time, count, source, keyword_type)
            VALUES ('{str(kwargs['keyword_name'])}', '{kwargs['job_id']}', '{kwargs['created_time']}', '{kwargs['count']}',
                    '{kwargs['source']}', '{kwargs['keyword_type']}')
            """
    MysqlManager(1).insert(query)
...
def insert_job_model_removeok(**kwargs):
    query = f"""
            INSERT INTO job_model_keywords_remoteok (keyword_name, job_id, created_time, count, source, keyword_type)
            VALUES ('{str(kwargs['keyword_name'])}', '{kwargs['job_id']}', '{kwargs['created_time']}', '{kwargs['count']}',
                    '{kwargs['source']}', '{kwargs['keyword_type']}')
            """
    MysqlManager(1).insert(query)
